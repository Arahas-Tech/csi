...
// const TempDashboard = ({ onDataChange, show, pSelectedLocation, pSelectedStartDate, pSelectedEndDate }) => {
...
//   const [envirotemp, setEnviroTemp]=useState([]);
//   const [enviroHumidity,setEnviroHumidity]=useState([]);
...
//           setEnviroNO2(NO2);
//           setEnviroco2(co2);
//           setEnviroHumidity(humidity);
//           setEnviroTemp(temp);
//           // Calculate the average AQI
//           if (filteredData.length > 0) {
//               const averageTemp = (filteredData.reduce((sum, item) => sum + item.temp, 0) / filteredData.length).toFixed(2);
//               const averageHumidity = (filteredData.reduce((sum, item) => sum + item.humidity, 0) / filteredData.length).toFixed(2);
//               setTempValue(averageTemp);
//               setHumidityValue(averageHumidity);
  
//               if (onDataChange) {
//                   onDataChange({ tempValue: averageTemp, humidityValue:averageHumidity });
//               }
//               setAqiStatus(getAqiStatus(averageTemp));
//           } else {
//               setAqiValue(null);
