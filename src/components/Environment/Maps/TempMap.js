...
const TempMap = ({ averageData, latestDate }) => {
  const mapRef = useRef();
  ...
  useEffect(() => {
    ...
    markers.forEach((marker) => {
      const averageTempHumidityForLocation = averageData[marker.location];
      const avgTemp = averageTempHumidityForLocation ? averageTempHumidityForLocation.avgTemp : undefined;
      const avgHumidity = averageTempHumidityForLocation ? averageTempHumidityForLocation.avgHumidity : undefined;
      const markerColor = averageTempHumidityForLocation ? 'red' : 'black';
      ...
      feature.setStyle(
        new Style({
          image: new Icon({
            src: `data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"><path fill="${markerColor}" d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5S10.62 6.5 12 6.5 14.5 7.62 14.5 9s-1.12 2.5-2.5 2.5z"/></svg>`,
    ...
    map.on('pointermove', function (event) {
      ...
      if (feature && feature.get('name')) { // Check if feature is present and has a name (marker)
        ...
        const avgTemp = feature.get('avgTemp');
        const avgHumidity = feature.get('avgHumidity');
        const location = feature.get('location') || ''; // Use default value if undefined
        const hasData = avgTemp !== undefined && avgHumidity !== undefined;
        const popupBackgroundColor = hasData ? 'rgba(255, 182, 193, 0.8)' : 'white'; // Light red shade if data is present, white if not
        ...
        let popupHTML = `
          <div>
            ...
            <p><strong> Temperature: </strong>${avgTemp !== undefined ? `${avgTemp}°C` : 'Not Available'}</p>
            <p><strong> Humidity: </strong>${avgHumidity !== undefined ? `${avgHumidity}%` : 'Not Available'}</p>
          </div>
        `;

        popupBody.innerHTML = popupHTML;
        popupContent.style.backgroundColor = popupBackgroundColor;
      } else {
